...
def load(sym):
    ...
    # fetch 18 months so 200d MA has enough history
    end   = datetime.today()
    ...
    # SPY for comparison (same window)
    ...
    # sector ETF for volatility comparison
    sector_etf_map = {
...
with st.spinner(f"Loading {ticker}…"):
    try:
        df_full, spy, info, fin, rev_est, earn_est, news, cal, rec_sum, insiders, sector_etf_sym, sector_rets = load(ticker)
...
# trim to last 12 months for most charts
...
# ── fundamental helper functions (used by assessment + charts) ─────────────────
...
def hist_series(fin_df, *keys):
    ...
    s = s.sort_index(); s.index = s.index.year
    return s.astype(float)

def est_series(est_df, row_key="avg"):
    if est_df is None or est_df.empty: return pd.Series(dtype=float)
    try:
...
def build_assessment():
    ...
    # ── short interest (high = squeeze risk, not necessarily bearish) ───────────
...
# ══════════════════════════════════════════════════════════════════════════════
# PRICE + MOVING AVERAGES + VOLUME + RSI
# ══════════════════════════════════════════════════════════════════════════════
...
# RSI
if not rsi.empty:
    rsi_colors = [UP_COL if v >= 50 else DOWN_COL for v in rsi.values]
    fig_ma.add_trace(go.Scatter(x=rsi.index, y=rsi.values, mode="lines",
        name="RSI (14)", line=dict(color="#333", width=1.8),
        fill="tozeroy",
        fillcolor="rgba(26,86,219,0.06)"), row=3, col=1)
    fig_ma.add_hline(y=70, line_color=DOWN_COL, line_dash="dash",
                     line_width=1, annotation_text="Overbought 70",
                     annotation_font_size=12, annotation_font_color=DOWN_COL, row=3, col=1)
    fig_ma.add_hline(y=30, line_color=UP_COL,   line_dash="dash",
                     line_width=1, annotation_text="Oversold 30",
                     annotation_font_size=12, annotation_font_color=UP_COL, row=3, col=1)
...
fig_ma.update_yaxes(title_text="RSI",       title_font=dict(size=13), row=3, col=1,
                    range=[0, 100])
...
if latest_ma50 and latest_ma200:
    ...
    ma_msg = (
        f"<b>50-day MA: ${latest_ma50:.2f}</b> &nbsp;|&nbsp; "
        f"<b>200-day MA: ${latest_ma200:.2f}</b><br><br>"
    )
    if above200 and above50:
        ma_msg += (
            f"Price is <b>above both moving averages</b> — a classically bullish setup. "
        ...
        ma_msg += (
            f"Price is <b>below the 50-day MA but above the 200-day MA</b>. "
        ...
        ma_msg += (
            f"Price is <b>below the 200-day MA but above the 50-day MA</b>. "
        ...
        ma_msg += (
            f"Price is <b>below both moving averages</b> — a bearish setup in the eyes of "
...
if drop_idx.empty:
    ...
    if outcomes_30:
        ...
        bin_w  = max(5, round(spread / 6 / 5) * 5)   # sensible bin width
...
if rec_mean is not None and n_analysts:
    if   rec_mean <= 1.5: cons_label = "Strong Buy";  cons_color = UP_COL
    elif rec_mean <= 2.5: cons_label = "Buy";          cons_color = "#2d9e5f"
    elif rec_mean <= 3.5: cons_label = "Hold";         cons_color = "#888800"
    elif rec_mean <= 4.5: cons_label = "Sell";         cons_color = "#cc6600"
    else:                 cons_label = "Strong Sell";  cons_color = DOWN_COL
...
if fin.empty:
    ...
    earn_hist = hist_series(fin, "Net Income", "Net Income Common Stockholders",
                            "Net Income From Continuing Operation Net Minority Interest")
    # derive forward net income estimate from EPS × shares outstanding
    ...
    st.plotly_chart(bar_chart(earn_hist, earn_fwd,
                              "Net Earnings", "Net Income ($)", fmt_billions=True),
                    use_container_width=True)
